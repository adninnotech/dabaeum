...
@Import({AttendanceApiMapper.class, CompletionApiMapper.class, ApiExceptionHandler.class,
    ClockConfiguration.class, SecurityConfiguration.class, JacksonConfiguration.class,
    RequestIdFilter.class})
class Stage5DocumentationAcceptanceTest {
    ...
    @Test
    void documentsNineOperationsAndCanonicalStageContract() throws Exception {
        assertThatStageContractAndAsciiDocArePresent();
        ...
        when(completionService.evaluate(any(), any())).thenReturn(completion(CompletionStatus.ELIGIBLE));
        when(completionService.confirm(any(), any())).thenReturn(completion(CompletionStatus.COMPLETED));

        mockMvc.perform(post("/api/v1/sessions/{id}/qr-token", SESSION_ID).with(managerUser()))
            .andExpect(status().isCreated()).andExpect(openApi().isValid(OpenApiContract.VALIDATOR))
            .andDo(document("attendance-qr-token-issue", preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(get("/api/v1/sessions/{id}/attendance", SESSION_ID).with(managerUser()))
            .andExpect(status().isOk()).andDo(document("attendance-session-list",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(post("/api/v1/sessions/{id}/attendance", SESSION_ID).with(managerUser())
                .contentType(MediaType.APPLICATION_JSON).content(adminAttendanceJson()))
            .andExpect(status().isCreated()).andDo(document("attendance-record",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(get("/api/v1/attendance/{id}", ATTENDANCE_ID).with(managerUser()))
            .andExpect(status().isOk()).andDo(document("attendance-get",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(patch("/api/v1/attendance/{id}", ATTENDANCE_ID).with(managerUser())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"status\":\"LATE\",\"reason\":\"현장 확인\"}"))
            .andExpect(status().isOk()).andDo(document("attendance-adjust",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(get("/api/v1/enrollments/{id}/attendance-summary", ENROLLMENT_ID)
                .with(managerUser()))
            .andExpect(status().isOk()).andDo(document("attendance-summary",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(get("/api/v1/enrollments/{id}/completion", ENROLLMENT_ID)
                .with(managerUser()))
            .andExpect(status().isOk()).andDo(document("completion-get",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(post("/api/v1/enrollments/{id}/completion/evaluate", ENROLLMENT_ID)
                .with(managerUser()).contentType(MediaType.APPLICATION_JSON)
                .content("{\"attendanceRate\":66.67,\"completedMinutes\":120,\"creditValue\":null}"))
            .andExpect(status().isOk()).andDo(document("completion-evaluate",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
        mockMvc.perform(post("/api/v1/enrollments/{id}/completion/confirm", ENROLLMENT_ID)
                .with(managerUser()))
            .andExpect(status().isOk()).andDo(document("completion-confirm",
                preprocessRequest(prettyPrint()), preprocessResponse(prettyPrint()), fields()));
    }

    private org.springframework.test.web.servlet.request.RequestPostProcessor managerUser() {
        return user("manager").roles("INSTITUTION_ADMIN");
    }

    private Snippet fields() {
        return relaxedResponseFields(fieldWithPath("data").description("API data envelope"));
    }

    private String adminAttendanceJson() {
        return "{\"enrollmentId\":\"33333333-3333-3333-3333-333333333333\","
            + "\"attendanceMethod\":\"ADMIN\",\"status\":\"PRESENT\","
            + "\"source\":\"ADMIN_WEB\"}";
    }
    ...
    private void assertThatStageContractAndAsciiDocArePresent() throws Exception {
        ParseOptions options = new ParseOptions();
        options.setResolve(true);
        var parsed = new OpenAPIV3Parser().readLocation(CONTRACT.toUri().toString(), null, options);
        org.assertj.core.api.Assertions.assertThat(parsed.getOpenAPI()).isNotNull();
        String adoc = Files.readString(Path.of("src/docs/asciidoc/stage5.adoc"));
        for (String snippet : List.of("attendance-qr-token-issue", "attendance-session-list",
            "attendance-record", "attendance-get", "attendance-adjust", "attendance-summary",
            "completion-get", "completion-evaluate", "completion-confirm")) {
            org.assertj.core.api.Assertions.assertThat(adoc).contains(snippet);
        }
    }
}
