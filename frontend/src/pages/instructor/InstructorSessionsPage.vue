  ...
  <q-page padding>
    ...
    <div class="app-page-header row items-center justify-between q-mb-lg">
      ...
      <div class="row q-gutter-sm">
        <q-btn outline color="primary" icon="refresh" label="새로고침" :loading="loading" @click="loadAll" />
      ...
      <template #body-cell-status="props">
        <q-td :props="props">
          <q-btn-dropdown
            flat
            dense
            :color="statusColor[props.row.status] || 'grey'"
      ...
      <template #body-cell-actions="props">
        <q-td :props="props">
          ...
          <q-btn
            ...
            color="primary"
            label="QR 발급"
            :loading="qrLoadingId === props.row.id"
            @click="issueQr(props.row)"
          />
        </q-td>
      </template>
    </q-table>
...
export default {
  ...
  computed: {
    ...
    courseTitle() {
      if (!this.course) return '회차';
      return `${this.course.courseCode || ''} ${this.course.title || ''}`.trim() || '회차';
    },
  },
  watch: {
    courseId: {
  ...
  methods: {
    ...
    async loadAll() {
      ...
      this.errorMessage = '';
      try {
        const [course, sessionsResult] = await Promise.all([
          getCourse(this.courseId),
          listCourseSessions(this.courseId, { page: 0, size: 100 }),
        ]);
        this.course = course;
        this.rows = Array.isArray(sessionsResult.data) ? sessionsResult.data : [];
      } catch (error) {
