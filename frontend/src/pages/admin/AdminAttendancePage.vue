  ...
  <q-page padding>
    <div class="app-page-header row items-center justify-between q-mb-lg">
      <div>
        ...
        <div class="app-page-header__subtitle text-grey-7">과정 → 회차 선택 · 출결 목록 · QR 발급 · 정정</div>
      </div>
        ...
        >
          ...
          <template #body-cell-actions="props">
            <q-td :props="props">
              ...
              <q-btn flat dense color="warning" label="지각" @click="adjust(props.row, 'LATE')" />
              <q-btn flat dense color="negative" label="결석" @click="adjust(props.row, 'ABSENT')" />
              <q-btn flat dense color="positive" label="출석" @click="adjust(props.row, 'PRESENT')" />
            </q-td>
          </template>
        </q-table>
      </div>
...
export default {
  ...
  methods: {
    ...
    async adjust(row, status) {
      try {
        await adjustAttendance(row.id, { status, reason: `관리자 정정: ${status}` });
        this.$q.notify({ type: 'info', message: `출결 정정: ${status}`, position: 'top' });
